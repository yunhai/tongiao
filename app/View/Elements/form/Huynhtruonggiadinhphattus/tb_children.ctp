<table class="table table-bordered col-xs-12 tb-generator">
    <tbody>
        ...
        foreach ($data as $key => $val) {
            ?>
            <tr>
                <?php if (!in_array($key, array('nkdtbdvtg', 'dqcthl', 'cdt', 'cacthongtinkhac', 'kiennghi'))) { 
                ?>
                <th class="col-xs-2 text-left">
                    <label class="control-label pull-right">
                        <?php 
                            if ($key == "noiohiennay_diachi") {
                                echo "Địa chỉ";
                ...
                <td >
                    <?php
                        if ($key == "Địa chỉ" || 
                            ...
                            echo $this->element("form/Huynhtruonggiadinhphattus/tb_children", array("data" => $val, "is_call" => 1));
                        } else if ($key == "Bố, mẹ đẻ") {
                            echo $this->element("form/Huynhtruonggiadinhphattus/tb_children_quanhe", array("data" => $val, "is_call" => 1));
                        ...
                        } else if ($key == "Quá trình hoạt động tôn giáo ở nước ngoài (nếu có)") {
                            echo $this->element("layout/table/tb_vertical_auto", array("data" => $val, "id" => "quatrinhhoatdongtongiaoonuocngoaineuco"));
                        } else if (in_array($key, array('nkdtbdvtg', 'dqcthl', 'cdt', 'cacthongtinkhac', 'kiennghi'))) {
